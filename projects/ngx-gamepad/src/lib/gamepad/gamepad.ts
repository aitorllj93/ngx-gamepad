...
const gamepad = {
  init(gpad) {
    const gamepadPrototype = {
      ...
      triggerDirectionalAction(id, axe, condition, x, index) {
        ...
        if (gps.length) {
          ...
          if (gp.axes) {
            ...
            for (let x = 0; x < this.axes * 2; x++) {
              const val = gp.axes[x + modifier].toFixed(4) as any;
      ...
      associateEvent(eventName, callback, type) {
        if (eventName.match(/^button\d+$/)) {
          const buttonId = parseInt(eventName.match(/^button(\d+)$/)[1], 10);
